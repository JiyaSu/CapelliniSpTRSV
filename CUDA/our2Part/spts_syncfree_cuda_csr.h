#ifndef _SPTS_SYNCFREE_CUDA_CSR_
#define _SPTS_SYNCFREE_CUDA_CSR_
...
//        printf("%d\n",global_id);
    ...
    i=global_id;
    int warp_begin=warp_begin = ((int)(i/WARP_SIZE))*WARP_SIZE;
    
    
        //printf("%d\n",i);
//        for(j=d_csrRowPtr[i];j<(d_csrRowPtr[i+1]);j++)
//        {
//            col=d_csrColIdx[j];
//            if(col<warp_begin)
//            {
////                        if(global_id<32)
////                            printf("111111111\n");
////            while(atomic_load_explicit((atomic_int*)&d_get_value[col],memory_order_acquire, memory_scope_device)!=1)
...
//        }
    ...
    for(j=d_csrRowPtr[i];j<(d_csrRowPtr[i+1]);j++)
    {
        ...
        //atomicAdd((VALUE_TYPE *)&d_x[i], 1);
        //atomicAdd((int *)&d_get_value[i], 1);
        
//        d_x[i] = 1;
//        d_get_value[i]=1;
        //if(i==-1)
            //printf("%d %g %d\n",i,d_x[i],d_get_value[i]);
        col=d_csrColIdx[j];
//        for(k=0;k<WARP_SIZE;k++)
        ...
        //else if(atomic_load_explicit((atomic_int*)&d_get_value[col],memory_order_acquire, memory_scope_device)==1 && col<global_id)
//            else if(col>i)
//                break;
        }
//        printf("end %d\n",i);
...
int spts_syncfree_cuda_csr(const int           *csrRowPtrL_tmp,
...
//    int *while_profiler = (int *)malloc(sizeof(int) * n);
    ...
    for (i = 0; i < BENCH_REPEAT; i++)
    {
        ...
        spts_syncfree_cuda_executor_csr<<< num_blocks, num_threads >>>
        (d_csrRowPtrL, d_csrColIdx, d_csrValL,
         d_get_value,m, nnzL, d_b, d_x,0,d_id_extractor);
        
//        int num_blocks2 = MAX_BLOCK;
//        gettimeofday(&t1, NULL);
//        for (j=0; j<num_blocks; j+=MAX_BLOCK)
//        {
//            //printf("%d %d\n",num_blocks2, num_threads);
//            if(j+MAX_BLOCK>num_blocks)
//                num_blocks2=num_blocks-j;
//            spts_syncfree_cuda_executor_csr<<< num_blocks2, num_threads >>>
//            (d_csrRowPtrL, d_csrColIdx, d_csrValL,
//             d_get_value,m, nnzL, d_b, d_x,j);
